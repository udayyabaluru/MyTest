...
class ProductControllerTest {
	...
	public void testvoiceanalyze() throws Exception { 
		...
		//mockMvc.perform(post("/voice/analyze",null)).andExpect(status().isOk());
		MockMultipartFile multipartFile = new MockMultipartFile("file", "human_voice_Script2.mp3",
				"text/plain", "Spring Framework".getBytes());
		this.mockMvc.perform(multipart("/voice/analyze").file(multipartFile))
				.andExpect(status().isFound())
				.andExpect(header().string("Location", "/voice/analyze"));
	} 
}
